<?php
	...
	$res=qry("select estado,nivel,hora_in,(concat(date(hora_in),' 23:59:59') + INTERVAL (TIMESTAMPDIFF(MONTH,concat(date(hora_in),' 23:59:59'),now())) month),deuda from user_nivel where iduser='$Xiduser'");
	
	//print "select estado,nivel,hora_in,(concat(date(hora_in),' 23:59:59') + INTERVAL (TIMESTAMPDIFF(MONTH,concat(date(hora_in),' 23:59:59'),now())) month),deuda from user_nivel where iduser='$Xiduser'";
	...
	if($data[0]>0){
		...
		{
			if($data[4]==0){
				$qry1="select sum(if(a.estado=1,1,0)),sum(if(a.estado=3,1,0)) from puntos  a, user_nivel b where a.login=b.login and  a.iduser='$Xiduser' and a.tipo=3 and a.fecha_venc>'$data[3]'";
				$res1 = qry($qry1);
			...
			$(window).load(function(){
				...
				$("#conf").click(function(){
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_unico',i:idbol,lp:$("#listpre").val()},function(data){
							$(".desc").html(data.text);
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv",dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest,lp:$("#listpre").val()},
					function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
