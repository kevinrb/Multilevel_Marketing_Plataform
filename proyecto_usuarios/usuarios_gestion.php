	...
	<head>
		...
		<script>
			...
			$(window).load(function(){
				...
				$(document).on("click","#busq",function(){
					var val=$("#vbusq").val();
					if(val!=null && val.trim()!="")
					{
						$.post("ajax/usuarios.php",{a:"busr",val:val},function(data){
							if(data[0]=="1"){ $("#work1").show(); $("#work2").hide(); $("#resb").html(data[1]); }
						},"json");
					}else{alert("Debe Especificar Un Valor"); $("#vbusq").focus();}
				});
				
				$(document).on("keyup","#vbusq",function(e){
					if(e.keyCode==13){ $("#busq").trigger("click"); }
				});
								...
								<div class="container">
									...
									<div class="col-sm-12">
										<div style="margin-bottom:10px;" class="input-group">
											<input id="vbusq" type="text" class="form-control" placeholder="Buscar...">
											<span class="input-group-btn">
												<button id="busq" class="btn btn-primary" type="button">Buscar</button>
												<button id="nusr" class="btn btn-success" type="button">Nuevo</button>
												<button id="gener" class="btn " type="button">General</button>
											</span>
										</div>
									</div>
									
									<div style="display:none;" class="col-sm-12" id="work1">
										<table class="table">
											<thead id="tres"><tr><th>LOGIN</th><th>Nombre</th><th>Apellidos</th><th>Campaña</th><th>IDUPL.</th><th>UPLINE</th></tr></thead>
											<tbody id="resb"></tbody>
										</table>
									</div>
