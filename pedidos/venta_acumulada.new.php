<?php
	...
	if($can!="0" && $lp!="0"){
		...
		$res=qry("select a.idprod,a.precio1,UPPER(a.nombre) nombre,1 pack,'' barras,0 stock,a.promo, '' precvar, '' fraccion,'' unmedida, '' unidades,if(a.codigo1 in ($idprosblo),1,0) from productos$Xpais a   where  isnull(a.nombre)!=1 and a.nombre!='' and a.estado=1 and prof=0 GROUP BY a.idprod order by a.nombre;");	
		$prod=array();
		while($temp=mysql_fetch_row($res)){ $prod[]=$temp; }
		$listp=json_encode($prod);
		}else{
		
		$qprod=qry("select a.idprod,a.precio1,UPPER(a.nombre) nombre,1 pack,'' barras,0 stock,a.promo, '' precvar, '' fraccion,'' unmedida, '' unidades,if(a.codigo1 in ($idprosblo),1,0) from productos$Xpais a where  isnull(a.nombre)!=1 and a.nombre!='' and a.estado=1 and prof=0  GROUP BY a.idprod order by a.nombre;");	
			...
			$(window).load(function(){
							...
							if(nres>0){
								for(i=0;i<lres.length;i++)
								{
									...
									if(lres[i][11]==1){
										...
										html=html+"<div "+st+" pos='"+(i+1)+"' stk='"+lres[i][5]+"' class='resultlist' barr='"+lres[i][4]+"' nom='"+lres[i][2]+"' fra='"+lres[i][8]+"' unm='"+lres[i][9]+"' unp='"+lres[i][10]+"' idpr='"+lres[i][0]+"' punit='"+lres[i][1]+"' precv='"+lres[i][7]+"' pack='"+lres[i][3]+"'>"+lres[i][2]+" - <?php echo $Xmoneda; ?> "+lres[i][1]+" - x"+lres[i][3]+"</div>";
									}
								}
								$("#listres").html(html);
				...
				$("#conf").click(function(){
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_acumulado',i:idbol,lp:$("#listpre").val()},function(data){
							$(".desc").html(data.text);
							$("#acum").text(data.acum);
				...
				$("#addpag").click(function(){
					...
					if(l<4){
						if(m>0){
							$("#listpag").append("<tr tip='"+t+"' class='listmnt' inf='"+m+"'><td>"+l+"</td><td>"+t+"</td><td><?php echo $Xmoneda; ?> "+m+"</td><td><button class='rmvpag btn btn-warning btn-xs'><span class='glyphicon glyphicon-remove'></span></button></td></tr>");
							$('#tabledv').show();
							$("#montotmp").val("");
						}else
						{
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv_acum",dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest,lp:$("#listpre").val()},function(data){
		...
		<div class="container">
			...
			<div class="row">
				<div class="panel panel-success">
					<div style="height:48px;" class="panel-heading">Lista Productos <strong>(<?php echo "Monto Acumulado Mensual: $Xmoneda ".$Xacum;?>)</strong></div> 
