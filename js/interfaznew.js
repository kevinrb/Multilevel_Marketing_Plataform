...
$("body").on("change","#clamotivo", function(){
	var valu=$(this).val();
	$.post("ajax/interfaz_ajaxnew.php",{tipo:"motivo",val:valu},function(data){
		$("#idmotivo").html(data);
...
$("#idcarga").change(function(){
	...
	//SE TRAEN LOS DATOS 
	$.post("ajax/interfaz_ajaxnew.php", {idcarga:idcarga}, function(data){
		$('#myTab').show();
		$("#datos").empty();
	...
	$('#timeagenda').datetimepicker({format: 'yyyy-mm-dd hh:mm:ss', showMeridian: true, autoclose: true, todayBtn: true, language:'es',minView:0});
});
...
$("body").on("click",".call", function(){
	...
	but.button('loading');
	$.post("ajax/interfaz_ajaxnew.php", {recall:tel, idops:ido,save:1 }, function(data){
	llamar(tel,ido,but);
	});
...
$("body").on("click",".call1", function(){
	...
	but.addClass('btn-default');
	but.button('loading');
	$.post("ajax/interfaz_ajaxnew.php", {recall:tel, idops:ido }, function(data){
	llamar1(tel,ido,but);
	});
