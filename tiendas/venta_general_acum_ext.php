<?php
	require("../sec.php");
	require("../ws/func.php");
	$result1=posturl(array("a"=>"series_exe","login"=>$Xlogin),$XXurl_erp2);
	$ser=json_decode($result1);
	$pe=$ser[0];
	$seltpago=$ser[3];
	$a_ser=json_encode($ser[1]);
	$a_cor=json_encode($ser[2]);
	
	$result=posturl(array("a"=>"stock_local_exe","login"=>$Xlogin),$XXurl_erp2);
	...
	$idcli=$idcli[0];
	$listp=array();
	$qprod=qry("select a.idprod,a.precio,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades,if(idprod in ($idprosblo),1,0) from productos a where isnull(a.nombre)!=1 and a.nombre!='' and ( a.porlocal=0 or (a.porlocal=1 and
	a.idprod in (select idprod from productolocal where local='$Xlocal'))) and a.estado=1 and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='14'),1,0),1) and insumo=0 GROUP BY a.idprod order by a.nombre;");
	...
	$tbody="";
	$i=1;
	$res=qry("select a.idprod,TRIM(UPPER(CONCAT(b.nombre,' ',b.nombre1,' ',b.nombre2,' ',b.nombre3,' ',b.nombre4))) nombre,a.cantidad,if(a.promo=0,a.precio,c.precio),(a.cantidad+if(a.fraccion/b.unidades is null,0,a.fraccion/b.unidades))*if(a.promo=0,a.precio,c.precio) total,a.promo,if(a.fraccion>0,CONCAT(a.fraccion,b.unmedida),''),(a.cantidad+(a.fraccion/b.unidades)),UPPER(a.cantidad+(a.fraccion/b.unidades)),a.canal,a.lista,a.localdst,FLOOR(a.cantidad/b.pack) packs,b.pack from bolscanal a,productos b,kitparts c where b.idprod=a.idprod and a.login='$Xlogin' and a.local='$Xlocal' and a.estado=1 and a.idbolsa='0' and if(a.promo=0,1,if(a.promo=c.idkit and a.idprod=c.idprod,1,0)) group by a.idprod,a.promo order by a.promo,nombre;");
	...
	while($temp=mysql_fetch_row($res))
	{
		...
		if($formap=="P"){$exb="<td>$temp[12]</td><td>$temp[13]</td><td>$temp[2]</td>";} 
		
		
	$tbody .= "<tr promo='$temp[5]' unit='$temp[3]' cant='$temp[7]' class='list $ex' idp='$temp[0]'><td>$i</td><td>$temp[1]</td>".$exb."<td>$temp[6]</td><td>$temp[3]</td><td>$temp[4]</td><td><button class='btn btn-warning remove btn-xs'><span class='glyphicon glyphicon-remove'></span></button></td></tr>"; $i++; $can=$temp[9]; $lp=$temp[10]; $loc=$temp[11];}
	...
	{ $adjs="$('#listcanales').val('$can'); $('#listcanales').attr('disabled',true); $('#listpre').val('$lp'); $('#listpre').attr('disabled',true); $('#listloc').val('$loc'); $('#listloc').attr('disabled',true);";
		$res=qry("select a.idprod,a.precio$lp,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades,if(idprod in ($idprosblo),1,0) from productos a where isnull(a.nombre)!=1 and a.nombre!=''  and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='$can'),1,0),1) and a.estado=1 GROUP BY a.idprod order by a.nombre;");
		$prod=array();
	...
	$res=qry("select login,concat(nombre,' ',apellidos) from usuarios where activo=1 and login>0  order by nombre");
	$usr="<option value='' selected disabled>--Elija--</option>";
	while($temp=mysql_fetch_row($res))
	{ $usr .= "<option value='$temp[0]'>$temp[0]-$temp[1]</option>";}
...
<html>
	<head>
		...
		<script>
			...
			var a_ser=<?php echo $a_ser;?>;
			var a_cor=<?php echo $a_cor;?>;
			var a_cli=[];
			var a_dest=[];
			...
			var domi=new Array();
			function buscaprod(idprod){r=false; $(".list").each(function(index){ if($(this).attr("idp")==idprod || $(this).attr("promo")==idprod){r=true;}}); return r;}
			function monto(){
			...
			$(window).load(function(){
				...
				///
				/////BUSCAR
				$(".busq2").keyup(function(e2){
					var listres2=$(this).parent().parent().find(".listres2");
					var busq2=$(this);
					if(e2.keyCode!=38 & e2.keyCode!=40 & e2.keyCode!=13){
						clearTimeout(bus);
						bus=setTimeout(function(){
							$.post("../ajax/gproc.php",{a:"fuser_act",s:busq2.val(),c:$("#listcanales :selected").text()},function(data){
								html="";
								for(i=0;i<data.length;i++)
								{ html=html+"<div idc='"+data[i][0]+"' nom1='"+data[i][1]+"' nom2='"+data[i][2]+"' dir='"+data[i][3]+"' dis='"+data[i][4]+"' ema='"+data[i][5]+"' cel='"+data[i][6]+"' tel='"+data[i][7]+"' class='listcli' pos='"+(i+1)+"' div>"+data[i][1]+","+data[i][2]+"( "+data[i][0]+" )"+"</div>"; }
								listres2.html(html);
								listres2.addClass("form-control");
								listres2.css({width:(parseInt(busq2.css("width")))+"px"});
								listres2.show();
								busq2.keydown(function(e){
									tecla=e.keyCode;
									if((tecla==38 | tecla==40))
									{
										if($(".reslistcli").length>0)
										{
											pos=$(".reslistcli").attr("pos");
											if(tecla==40){sig=parseInt(pos)+1;}
											else{sig=parseInt(pos)-1;}
											if(sig==0){ sig=$(".listcli").length; }  
											if(sig== ($(".listcli").length+1)){sig=1;}
											$(".listcli").removeClass("reslistcli");
											$('div[pos='+sig+']').addClass("reslistcli");
										}
										else
										{$($(".listcli")[0]).addClass("reslistcli");}
									}					
									if(tecla==13)
									{
										$("#logcl").hide(); 
										obj=$(".reslistcli");
										idcli=obj.attr("idc");
										nomcom=obj.attr("nom1")+" "+obj.attr("nom2");
										busq2.val(nomcom);
										busq2.attr("login",idcli);
										busq2.attr("lbl",nomcom);
										$("#nombre").val(obj.attr("nom1"));
										$("#apellido").val(obj.attr("nom2"));
										$("#direccion").val(obj.attr("dir"));
										$("#distrito").val(obj.attr("dis"));
										$("#correo").val(obj.attr("ema"));
										$("#telefono").val(obj.attr("cel"));
										$("#celular").val(obj.attr("tel"));
										$(".listres2").hide();
										$("#taginfo").html("Datos del Cliente");
										$('#dvsavecl').hide();
										$("#namem").text(obj.attr("nom1")+","+obj.attr("nom2"));
										////calculo de niveles//
										$("#idruc").html("");
										$("#iddest").html("");
										///////Empty RUC
										...
										$(".indest").prop('disabled',false);
										$("#iddest").val('');								
										$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_acumulado',tipo:"g",i:idbol,login:idcli},function(data){
											$(".desc").html(data.text);
											a_cli=data.a_cli;
											a_dest=data.a_dest;
											$("#idruc").html(data.selcli);
											$("#iddest").html(data.seldest);
											$("#acum").text(data.acum);
											$("#socio").text(idcli);
											...
											data.total=data.total-data.desc;
											data.total=Math.round(100*data.total)/100;
											data.total=data.total.toFixed(2);
											$("#totmon").val(data.total);
											$("#total").val(data.total)
											$("#totalf").text(data.total)									
										...
										///////	
										$("#docim").text("("+obj.attr("idc")+")");
										$("#showinf").attr("disabled",false);
										$("#dvsavecl").html("<button class='btn btn-success' id='updtcl'>Actualizar</button>");
										$("#infclnn").hide();
										$("#infclr").show();
										$("#infocl").hide();
										$("#showinf").attr("std","0");
										$("#showinf").children().attr("class","glyphicon glyphicon-chevron-down");
									}
				...
				$(document).on("click",".reslistcli",function(){
					var busq2=$(this).parent().parent().find(".busq2");
					$("#logcl").hide(); 
					obj=$(".reslistcli");
					idcli=obj.attr("idc");
					nomcom=obj.attr("nom1")+" "+obj.attr("nom2");
				...
				$(document).on("click",".remove",function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{i:idbol,a:"qp",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				
				$(document).on("click",".plus",function(){
					p=$(this).parent().parent().attr("idp");
					m=$(this).parent().parent().attr("promo");
					$.post("../res/pedidos/venta_canales_auto.php",{i:idbol,a:"plus_prod",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				
				$(document).on("click",".less",function(){
					p=$(this).parent().parent().attr("idp");
					m=$(this).parent().parent().attr("promo");
					$.post("../res/pedidos/venta_canales_auto.php",{i:idbol,a:"less_prod",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				...
				/*
					$("#vend1").change(function(){
					$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_general',i:idbol,socio:$(this).val()},function(data){
					$(".desc").html(data.text);
					$("#idcl1").html(data.opcli);
					domi[0]=data.dir;
					domi[1]=data.ref;
					domi[2]=data.dep;
					domi[3]=data.prov;
				...
				function cambio_trans(){
					if($("#transp").is(':checked')){
						////calculo de niveles//
						var dvr=[];
						var vend1=$('#vend1').attr("login");; dvr.push([1,$('#vend1'),'num',0]);
						/*if(tr(dvr)==false){
							return false;
						}	*/
						$("#envd").show();
					...
					if(r==true){
						...
						{	
							...
							if($('#iddest').val()==null){
								...
								dep=$('#depa').val(); dvr.push([1,$('#depa'),'',0]);
								pro=$('#prov').val(); dvr.push([1,$('#prov'),'',0]);
								dis=$('#dist').val(); dvr.push([1,$('#dist'),'',0]);
							}
							else{
						...
						/////
						pe=$('#pe').val(); dvr.push([1,$('#pe'),'',0]);
						/////PAGO
						$('.pago').each(function(i){
							$(this).parent().removeClass('has-error');
							tpago[i]=$(this).parent().find('.tpago').val();
							pago[i]=$(this).parent().find('.pago').val();
							totalp=totalp+parseFloat($(this).val());
							if(!($(this).val()>0) && $('#total').val()!=0)
							{
						...
						totalp=Math.round(100*totalp)/100;
						if($('#total').val()<=totalp)
						{
							$('#spantot').removeClass('alert-danger');
							$('#spantot').addClass('alert-success');
						}
						else{
							$('#tipopago').parent().addClass('has-error');
							btn.button('reset');		
							return false;
						}
						if(iden==1){
						...
						$.post("../res/pedidos/venta_canales_auto_ext.php",{
							a:"sv_transporte_acum",
							tipo:"g",
							login:idcli,
							idbol:idbol,
							fpago:$("#fpago").val(),
							idruc:idruc ,razon:razon ,dir_ruc:dir_ruc ,ruc:ruc ,dir:dir ,ref:ref ,dep:dep,pro:pro,dis:dis,iddest:iddest,tpago:tpago,pago:pago,transp:transp,pe:pe
							},function(data){
							if(data=="1"){
								alert("Pedido Enviado");location.reload();
								}else{
								btn.button('reset');
								alert(data);btn.button('reset');
							}
						});
					}
					else{}	
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv_acum",login:idcli,dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest},
					function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
				...
				$("#div_anul").hide();
				$("#pe").change(function(){
					var idpun=$("#pe option:selected").data("idpunto");
					var tipo=$("#pe option:selected").data("tipo");
					$("#nserie").text(a_ser[idpun]);
					$("#ncorr").text(a_cor[idpun]);
					if(tipo!="TI"){
						$("#div_anul").show();
						}else{
						$("#div_anul").hide();
				...
				$("#tipopago").change(function(){
					$("#add_pago").tooltip('show');
				});
				
				$(document).on("click","#add_pago",function(){
					var tpag=$("#tipopago").val();
					var totalp=0;
					var iden=0;
					...
					var totalpt=Math.round(100*$("#total").val())/100;
					if(totalpt<=totalp)
					{
						$("#spantot").removeClass("alert-danger");
						$("#spantot").addClass("alert-success");
					...
					if(tpag=="" || tpag==null)
					{
						...
						$("#tipopago").parent().removeClass('has-error');
						$("#div_pago").append("<div class='col-sm-6'><div class='input-group input-group-sm'><span class='input-group-addon'>"+tpag+"<input type='hidden' name='tpago[]' class='tpago' value='"+tpag+"'></span><input name=pago[] type='text' class='form-control pago' placeholder='Monto' value='"+(Math.round(100*(totalpt-totalp))/100)+"'><span class='input-group-btn'><button data-toggle='tooltip' data-placement='top' title='Eliminar Pago' class='btn btn-warning calendar btn-sm' type='button' id='remove_pago' ><span class='glyphicon glyphicon-remove'></span></button></span></div></div>");
						$("#tipopago").val('');
						$('[data-toggle="tooltip"]').tooltip()		
					}
				});
				...
				$(document).on("click","#remove_pago",function(){
					var divpag=$(this).parent().parent().parent();
					divpag.remove();
					var totalp=0;
					$(".pago").each(function(){
						$(this).parent().removeClass('has-error');
					...
					if($("#total").val()>totalp)
					{
						$("#spantot").removeClass("alert-success");
						$("#spantot").addClass("alert-danger");
						return false;
					}
				});
	...
	<body>
		<div class="container">
			...
			<div class="row">
				...
				<input id="listpre" type='hidden' value='' />
			...
			<div class="row">
				...
				<div class="col-sm-2 pull-right"><input disabled class="form-control input-sm" value="0" id="total"></div>
			</div>
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header"> 
						<h3 class="modal-title" id="myModalLabel">Informacion del Pago (Socio:<span id='socio'></span> Acum.:<span id="acum"></span>)</h3>
					</div>
						...
						<div class="row">
							<div class="col-md-12">	
								<div class="form-group col-sm-6 nm">
									<label>Socio</label>
									<div><input id="vend1" class='form-control input-sm busq2'  login='' lbl='' /></div>
									<div style='position:absolute; z-index:9;' class='listres2'></div>
								</div>
								<div class="form-group">
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="col-sm-12">
											...
											<div class="form-group adic"><label for="razon">Razon Social*</label><input onkeyup="javascript:this.value=this.value.toUpperCase();" required class="form-control input-sm dataruc" type="text" name="razon" id="razon" value=""></div>
											<div class="form-group adic"><label for="dir_ruc">Direccion*</label><input onkeyup="javascript:this.value=this.value.toUpperCase();" required class="form-control input-sm dataruc" type="text" name="dir_ruc" id="dir_ruc" value=""></div>
								...
								<div class="form-group">
									<label for="idruc">FORMA DE PAGO:</label>
									<div class="input-group">
										<select class="form-control input-sm" id="tipopago" name="tipopago"><?php echo $seltpago;?></select>
										<span class="input-group-btn">
											<button title="Click para Agregar Pago" class="btn btn-info calendar btn-sm" type="button" id="add_pago" data-toggle="tooltip" data-placement="top"><span class="glyphicon glyphicon-plus"></span></button>
										</span>
										<span class='input-group-addon alert-danger' id="spantot">
											<strong>Total : </strong><span id="totalf"></span>
										</span>
									</div>	
								</div>
								<div class="row">
									<div class="col-sm-12">
										<div class="col-sm-12" id="div_pago">
											<?php echo $tpagos;?>
										</div>
									</div>
								</div>
								<div class="form-group col-sm-10">
									<label for="pe">COMPROBANTE:</label>
									<div class="input-group">
										<select id='pe' class='form-control input-sm'><?php echo $pe;?></select>
										<span class='input-group-addon alert-info' >Nro: <span id="nserie"></span>-<strong><span id="ncorr"></span></strong></span>							
									</div>
								</div>						
							</div>
						</div>
