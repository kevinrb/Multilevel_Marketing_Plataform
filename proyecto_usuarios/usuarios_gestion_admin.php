<?php
	...
	if($d_val!="" )
	{
		...
		$res=qry("select a.login,a.nombre,a.apellidos,a.activo,TRIM(UPPER(CONCAT(if(isnull(a.apellidos),'',a.apellidos),' ',if(isnull(a.nombre)=1,'',a.nombre)))),d.nombre,f.iduser,f.nombre,a.iduser,g.iduser,g.nombre from usuarios a left join user_nivel b on b.iduser=a.iduser left join niveles_red d on d.idnivel=b.nivel left join asociadas e on e.idsocio=a.iduser left join usuarios f on f.iduser=e.idempresa  left join usuarios g on g.iduser=e.idpatro  where ((a.login like '%".$d_val."%') or (a.nombre like '%".$d_val."%') or (a.apellidos like '%".$d_val."%') or (a.iduser like '%".$d_val."%')) limit 30;");
		$htm="";
		if(mysql_num_rows($res)>0)
		{	
			while($temp=mysql_fetch_row($res))
			{$htm.="<tr class='showd' inf='".$temp[0]."' nom='".$temp[4]."' ><td>$temp[8]</td><td><a href='user_data.php?login=$temp[0]&iduser=$temp[8]'>".$temp[0]."</a></td><td>".$temp[1]."</td><td>".$temp[2]."</td><td>".$temp[5]."</td><td>".$temp[6]."</td><td>".$temp[7]."</td><td>".$temp[9]."</td><td>".$temp[10]."</td></tr>";}
		}
		else
		{
							...
							<body>
								<div class="container">
									...
									<div  class="col-sm-12" id="work1">
										<table class="table">
											<thead id="tres"><tr><th>ID</th><th>Login</th><th>Nombre</th><th>Apellidos</th><th>Campaña</th><th>IDUPL.</th><th>UPLINE</th><th>IDPAT.</th><th>PATROCINADOR</th></tr></thead>
