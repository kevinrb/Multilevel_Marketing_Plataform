<?php
	...
	$idcli=$idcli[0];
	$listp=array();
	$qprod=qry("select a.idprod,a.precio,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades from productos a where isnull(a.nombre)!=1 and a.nombre!='' and porlocal=0 and a.estado=1 and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='9'),1,0),1) and a.categoria=1 and insumo=0 GROUP BY a.idprod order by a.nombre;");
	...
	$res=qry("select login,concat(nombre,' ',apellidos) from usuarios where activo=1 and login>0 and now() between  ffii and date_add(ffii, interval 3 month) order by nombre");
		...
		<script>
			...
			$(window).load(function(){
				...
				$("#addprod").click(function(){
					...
					{$.post("../res/pedidos/venta_canales_afi.php",{i:idbol,a:"ap",c:c,p:objte[0],de:de,fr:$("#esfraccion").val(),cfr:$("#cantf").val(),can:$("#listcanales").val(),lp:$("#listpre").val(),loc:$("#listloc").val()},function(data){
						$("#listprod").html(data);
						$("#cant").val("1");
						$("#infprod").hide();
						$("#mntacm").val(monto());
				...
				$(document).on("click",".ped",function(){
					obj=$(this).attr("inf");
					$.post("../res/pedidos/venta_canales_afi.php",{a:"db",i:obj},function(data){
						idbol=obj;
						$("#listprod").html(data);
					});
					$("#mntacm").val(monto());
				...
				$(document).on("click",".remove",function(){
					...
					$.post("../res/pedidos/venta_canales_afi.php",{i:idbol,a:"qp",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				
				$(document).on("click",".plus",function(){
					p=$(this).parent().parent().attr("idp");
					m=$(this).parent().parent().attr("promo");
					$.post("../res/pedidos/venta_canales_afi.php",{i:idbol,a:"plus_prod",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				
				$(document).on("click",".less",function(){
					p=$(this).parent().parent().attr("idp");
					m=$(this).parent().parent().attr("promo");
					$.post("../res/pedidos/venta_canales_afi.php",{i:idbol,a:"less_prod",p:p,m:m},function(data){$("#listprod").html(data); $("#mntacm").val(monto());});
				});
				
				
				$("#save").click(function(){
					$.post("../res/pedidos/venta_canales_afi.php",{a:"sb"},function(data){if(data[0]=="1"){ idbol="0"; $("#listb").html(data[2]); 
				...
				$("#canc").click(function(){
					$.post("../res/pedidos/venta_canales_afi.php",{a:"bb",i:idbol},function(data){
				...
				$("#vend1").change(function(){
					$.post("../res/pedidos/venta_canales_afi.php",{a:'descuento_general',i:idbol,socio:$(this).val()},function(data){
						$(".desc").html(data.text);
				...
				$(document).on("click","#transp",function(){
					if($("#transp").is(':checked')){
						////calculo de niveles//
						var dvr=[];
						var vend1=$('#vend1').val(); dvr.push([1,$('#vend1'),'num',0]);
						if(tr(dvr)==false){
							return false;
						}
						...
						$("#vend1 option").attr("disabled",true);
						$("#vend1 option[value='"+vend+"']").attr("disabled",false);
						/*$.post("../res/pedidos/venta_canales_afi.php",{a:'descuento_transporte_general',i:idbol,socio:$("#vend1").val()},function(data){
							$(".desc").html(data.text);
							$("#totmon").val(data.total);
							$("#totpeso").val(parseFloat(data.peso));
							//alert(parseFloat(data.peso));
						...
						$("#totenv").val(0);
						/*$.post("../res/pedidos/venta_canales_afi.php",{a:'descuento_general',i:idbol,socio:$("#vend1").val()},function(data){
							$(".desc").html(data.text);
							$("#totmon").val(data.total);
							$("#totpeso").val(parseFloat(data.peso));
							//alert(parseFloat(data.peso));
				...
				$("#savev").click(function(){
					...
					if(r==true){
						...
						var dvr=[];
						var vend1=$('#vend1').val(); dvr.push([1,$('#vend1'),'num',0]);
						var ruc=$('#idcl1').val(); dvr.push([1,$('#idcl1'),'',0]);
						...
						$.post("../res/pedidos/venta_canales_afi.php",{
							a:"sv_transporte",
							idbol:idbol,
							fpago:$("#fpago").val(),
							idcli:$("#vend1").val(),
							...
							dis:dis,
							ruc:ruc
							},function(data){if(data=="1"){ alert("Pedido Enviado");location.reload();
							}});
					}
					else{}
				});
				...
				$('#datacliente').submit(function(event){
					...
					$.post("../res/pedidos/venta_canales_afi.php",{a:"newcl",data:data,canal:$("#listcanales :selected").text()},function(fff){
				...
				$("#listcanales").change(function(){
					$.post("../res/pedidos/venta_canales_afi.php",{a:"filprod",data:$(this).val(),lp:$("#listpre").val()},function(data){
						listp=data;
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_afi.php",{a:"calcenv",dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,val:val,i:idbol,idc:idc,tip:"g"},
					function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
						...
						<div class="row">
							<div class="col-md-12">	
								...
								<div class="form-group col-sm-6 nm">
									...
									<select id="fpago" class='form-control input-sm'><option value='' selected disabled>--Elija--</option><option>DEPOSITO</option><option>EFECTIVO</option></select>
								</div>	
								<div class="row nb">
									<div class="form-group col-sm-6 nm">
									</div>
