			...
			$(window).load(function(){ $('#tabledv').hide(); $('#dvsavecl').hide(); $('#logcl').hide();
				...
				$("#conf").click(function(){
					...
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_afi.php",{a:'descuento',i:idbol},function(data){
							$(".desc").html(data.text);
				...
				$(document).on("click","#transp",function(){
					if($("#transp").is(':checked')){
						if($("#vend1").val()>0)
						{
							...
							/*$.post("../res/pedidos/venta_canales_afi.php",{a:'descuento_transporte',i:idbol},function(data){
								$(".desc").html(data.text);
								$("#totmon").val(data.total);
								$("#totpeso").val(parseFloat(data.peso));
