<?php
	...
	$idcli=$idcli[0];
	$listp=array();
	$qprod=qry("select a.idprod,a.precio,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades,if(a.idprod in ($idprosblo),1,0) from productos a left join promo_puntos b on b.idprod=a.idprod where isnull(a.nombre)!=1 and a.nombre!='' and if(a.promo=0,1,a.idprod in (6933,6934,6940,6943,6936,6937,6938,6939,6940,7042,7039,7037,7033,7041,7038,7036,7035,7034,7093,7094,7095,7096,7097,7047) or b.prof in (0,1,3)) and ( a.porlocal=0 or (a.porlocal=1 and
	...
	{ $adjs="$('#listcanales').val('$can'); $('#listcanales').attr('disabled',true); $('#listpre').val('$lp'); $('#listpre').attr('disabled',true); $('#listloc').val('$loc'); $('#listloc').attr('disabled',true);";
		$res=qry("select a.idprod,a.precio$lp,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades,if(a.idprod in ($idprosblo),1,0) from productos a left join promo_puntos b on b.idprod=a.idprod where isnull(a.nombre)!=1 and a.nombre!=''  and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='$can'),1,0),1) and a.estado=1 and if(a.promo=0,1,a.idprod in (6933,6934,6940,6943,6936,6937,6938,6939,6940,7042,7039,7037,7033,7041,7038,7036,7035,7034,7093,7094,7095,7096,7097,7047) or b.prof in (0,1,3)) GROUP BY a.idprod order by a.nombre;");
		$prod=array();
		...
		<script>
			...
			$(window).load(function(){
							...
							$.post("../ajax/gproc.php",{a:"fuser_act",s:busq2.val(),c:$("#listcanales :selected").text()},function(data){
								...
								busq2.keydown(function(e){
									...
									if(tecla==13)
									{
										...
										$(".indest").prop('disabled',false);
										$("#iddest").val('');								
										$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_unico',tipo:"g",i:idbol,login:idcli},function(data){
											$(".desc").html(data.text);
											a_cli=data.a_cli;
											a_dest=data.a_dest;
				...
				$("#savev").click(function(){
					...
					if(r==true){
						...
						$.post("../res/pedidos/venta_canales_auto_ext.php",{
							...
							idbol:idbol,
							fpago:$("#fpago").val(),
							idruc:idruc ,razon:razon ,dir_ruc:dir_ruc ,ruc:ruc ,dir:dir ,ref:ref ,depa:depa ,prov:prov ,dist:dist,iddest:iddest,tpago:tpago,pago:pago,transp:transp,pe:pe
							},function(data){
							if(data=="1"){
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv",login:idcli,dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest},
					function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
							...
							<div class="col-md-12">	
								...
								<div class="row">
									...
									<!--<div id="divmov" class="col-sm-12">
										...
										<input type="checkbox" class="" id="transp" value="1" />
										</span>
