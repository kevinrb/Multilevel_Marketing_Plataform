<?php
	...
	/*
		...
		if($data[1]>0)
		{
		$qry1="select count(*) from puntos  a, usuarios b where a.login=b.login and  a.login='$Xlogin' and a.estado=1 and a.tipo=3 and if(day(now())>day('$data[2]'),a.hora>concat('2015-',month(now()),day('$data[2]',' 23:59:59')),a.hora>concat('2015-',(month(now())-1),day('$data[2]')))";
		$res1 = qry($qry1);
	...
	$idcli=$idcli[0];
	$listp=array();
	$qprod=qry("select a.idprod,a.precio8,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades from productos a where isnull(a.nombre)!=1 and a.nombre!='' and a.promo=0 and porlocal=0 and a.estado=1 and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='9'),1,0),1) and insumo=0 GROUP BY a.idprod order by a.nombre;");
	...
	{ $adjs="$('#listcanales').val('$can'); $('#listcanales').attr('disabled',true); $('#listpre').val('$lp'); $('#listpre').attr('disabled',true); $('#listloc').val('$loc'); $('#listloc').attr('disabled',true);";
		$res=qry("select a.idprod,a.precio$lp,TRIM(UPPER(CONCAT(a.nombre,' ',a.nombre1,' ',a.nombre2,' ',a.nombre3,' ',a.nombre4))) nombre,a.pack,if(isnull(a.barras)=1,'',a.barras) barras,0 stock,a.promo,a.precvar,a.fraccion,a.unmedida,a.unidades from productos a where isnull(a.nombre)!=1 and a.nombre!=''  and if(a.porcanal=1,if(a.idprod in (select idprod from prodcanal where canal='$can'),1,0),1) and a.estado=1 GROUP BY a.idprod order by a.nombre;");
		$prod=array();
			...
			$(window).load(function(){
				...
				$("#conf").click(function(){
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_adic',i:idbol},function(data){
							$(".desc").html(data.text);
				...
				$(document).on("click","#transp",function(){
					if($("#transp").is(':checked')){
						...
						$("#depa").val(domi[2]);
						$("#prov").html("<option>"+domi[3]+"</option>");
						$("#dist").html("<option>"+domi[4]+"</option>");
						/*$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_transporte_general',i:idbol,socio:$("#vend1").val()},function(data){
							$(".desc").html(data.text);
							$("#totmon").val(data.total);
							$("#totpeso").val(parseFloat(data.peso));
							//alert(parseFloat(data.peso));
						...
						$("#totenv").val(0);
						/*$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_general',i:idbol,socio:$("#vend1").val()},function(data){
							$(".desc").html(data.text);
							$("#totmon").val(data.total);
							$("#totpeso").val(parseFloat(data.peso));
							//alert(parseFloat(data.peso));
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv",dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,val:val,i:idbol,idc:idc,tip:"g"},
					function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
	...
	<body>
		<div class="container">
			...
			<div class="row">
				...
				<input id="listpre" type='hidden' value='8' />
