...
alert('Direccion de Correo Invalida');
	...
	//$.post('res/usuarios/proc.php',{tar:tar, tipo:tipo, id:id, est:"0" },function(data){});
	 ...
	 $.ajax({
							...
							data: {login:ll, nombre: nn, apellidos:ap, dni:d, email:mail, direc:dir, ffnn:fn, telefono:tel, celular:cel  , nivel:niv , edobs:obs, exp:ex, ffii:fi, sk:items1, ro:items2, gr:items3, css:c1, crr:c2, cgg:c3, loc:loc, tar:cc},
							success: function(){
							$('#verifica').removeClass("glyphicon-remove");
							$('#verifica').addClass("glyphicon-ok");
							$('#veri').removeClass("btn-danger");
