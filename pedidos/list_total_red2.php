<?php
	...
	if(isset($time_start)){
		...
		while($r = mysql_fetch_row($res)) {
			...
			/*FOR($i=1;$i<11;$i++){
				$tt=$i+10;
				$tan2.=" and nivel$i<={$r[$tt]}";	
			}
			$tan1="";*/
			$res2=qry("select d.fecha,e.nivel,d.nivel,d.idsocio_from,concat(a.nombre,' ',coalesce(a.apellidos,'')),concat(if(d.tipo=1,'AUTOCONSUMO',if(d.tipo=2,'ADICIONAL',''))),d.total,d.porcentaje,d.comision,round((comision),2)$tan from (usuarios a, user_nivel_day b,comisiones_historial_puntos3 d) left join niveles_red c on c.idnivel=b.nivel left join user_nivel_day e on e.fecha=d.fecha and e.login=b.login where d.iduser=b.iduser and a.iduser=d.idsocio_from and b.hora_in>'2015-01-01' and d.$qry_day and  b.iduser='$r[10]' and d.fecha>'2017-05-02' and b.fecha=last_day('$time_start-01') order by d.fecha");
			...
			while($rr=mysql_fetch_row($res2)){
				...
				FOR($i=1;$i<11;$i++){
					$tt=$i+9;
					$subto[$i]+=$rr[$tt];
					$tan1.=" and nivel$i<='{$rr[$tt]}'";	
				}
			}			
			
			$pais=mysql_fetch_row(qry("select b.pais from usuarios a left join paises b on a.idpais=b.idpais where iduser='$r[10]'"));
			...
			$bono=mysql_fetch_row(qry("select bono,'$r[10]' from bono_autos where 1 $tan1 order by nivel desc limit 1"));
			//$bono2=mysql_fetch_row(qry("select bono from bono_autos where 1 $tan2 order by nivel desc limit 1"));
			if($bono[0]>0){
				//echo "$r[10]---select bono from bono_autos where 1 $tan1 order by nivel desc limit 1<br />";
				qry("insert ignore into comisiones_adicional(fecha,login,tipo,comision,iduser) select '$time_start-15',login,'AUTO','$bono[0]',iduser from usuarios where iduser='$r[10]'");
			}
			$bono=mysql_fetch_row(qry("select sum(comision) from comisiones_adicional where fecha between '$time_start-01' and last_day('$time_start-01') and tipo='AUTO' and iduser='$r[10]'"));
			...
			$tbl.="<tr><td>$pais[0]</td><td>$r[5]</td><td $tempo>$r[0]</td><td $tempo>$r[1]/$r[10]</td><td $tempo>$max[0]($max[1])</td><td $tempo>$r[3]</td><td $tempo>$r[4]</td><td $tempo>$r[6]</td><td >$r[7]</td><td >$r[8]</td><td >$bono[0]</td><td >$bonoadi[0]</td><td >$bonobon[0]</td><td >$bonocorre[0]</td><td >".($r[9]+$bono[0]+$bonoadi[0]+$bonobon[0]+$bonocorre[0])."</td><td $tempo><input type='button' class='det btn btn-sm' data-login='$r[1]' data-iduser='$r[10]' data-fecha='$time_start-01' value='Det.' data-toggle='modal' data-target='#myModal' /> <input type='button' class='tree btn btn-sm' data-login='$r[1]' data-iduser='$r[10]' value='Arb.' data-toggle='modal' data-target='#myModal' /> <input type='button' class='venta btn btn-sm' data-login='$r[1]' data-iduser='$r[10]' value='Vent.' data-toggle='modal' data-target='#myModal' /> <input type='button' class='correc btn btn-sm btn-warning' data-login='$r[1]' data-iduser='$r[10]' value='CORREC' data-fecha='$time_start' data-toggle='modal' data-target='#myModal' /></td></tr>";
		}
		...
		<div class="">
			...
			<div class="panel panel-success">
				...
				<table class='table table-condensed' id='tablee'>
					<thead><th>PAIS</th><th>VC</th>
						<th>Nombre</th><th>Login</th><th>Nivel</th><th>Corte</th><th>PROF</th><th>T. PROF</th><th>C. Autoconsumo</th><th>C. Adic.</th><th>B. AUTO</th><th>UTIL.</th><th>B. DIAM.</th><th>B. CORREC.</th><th>C. Total</th><th>Detalle</th>
					</thead>
