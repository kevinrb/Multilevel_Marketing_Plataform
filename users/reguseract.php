<?php
	...
	$emp="";
	while($temp=mysql_fetch_row($res))
	{$sel=""; if($temp[0]==$Xlogin){$sel="selected";} $emp .= "<option $sel value='".$temp[0]."'>".$temp[1]."($temp[0])</option>";}
...
<html>
	<head>
		...
		<script>
			...
			$.post("../res/users/regnew.php",{a:"old",dni:dni,nom:nom,ape:ape,ema:ema,dir:dir,depa:depa,prov:prov,dist:dist,anio:anio,mes:mes,dia:dia,tel:tel,cel:cel,emp:emp,loc:loc,cue:cue,vou:vou,mon:mon,aniod:aniod,mesd:mesd,diad:diad,pago:pago},function(data){
			rst=data[0];
			msj=data[1];
		...
		<div class="container">
			...
			<div class="col-sm-6">
				...
				<div class="form-group">
					...
					<select id="pago" class="form-control"><option disabled selected >--Seleccione--</option><option value="NN">ANTIGUO</option><option value="DEPOSITO">DEPOSITO</option><option value="EFECTIVO">EFECTIVO</option></select>
				</div>
