<?php
	...
	*/
	...
	$res=mysql_query("select b.login , concat(b.nombre,' ',coalesce(b.apellidos,'')) from usuarios b where b.nombre!='' and isnull(b.nombre)!=1 and nivel in ('RED','VENTAS','ESPECIAL') order by nombre;");
	$emp="";
	while($temp=mysql_fetch_row($res))
	...
	$loc="";
	while($temp=mysql_fetch_row($res))
	{$sel=""; if($temp[0]=="AlmacenMultinivel"){$sel="selected";} $loc .= "<option $sel value='".$temp[0]."'>".$temp[0]."</option>";}
...
<html>
	<head>
		...
		<script>
			...
			$.post("../res/users/regnewred.php",{a:"old",dni:dni,nom:nom,ape:ape,ema:ema,dir:dir,depa:depa,prov:prov,dist:dist,anio:anio,mes:mes,dia:dia,tel:tel,cel:cel,emp:emp,loc:loc,ting:ting,sex:sex},function(data){
			rst=data[0];
			msj=data[1];
		...
		<div class="container">
			...
			<div class="col-sm-6">
				...
				<div class="form-group">
					...
					<input  id="emp" class="form-control" id="datalisttestinput" list="stuff" ></input>
					<datalist id="stuff">
						<?php echo $emp;?>
					</datalist>
				...
				<select id="ting" class="form-control" ><option value="RED">SOCIOS</option><option value="VENTAS">PROMOTORES</option><option value="ESPECIAL">ESPECIAL</option></select>
				<!--
					<h3>Tipo de Pago</h3>
					<hr size="2"/>
					<div class="form-group">
