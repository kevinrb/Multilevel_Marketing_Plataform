<?php
	...
	$qry = "select  a.login, a.nombre, a.apellidos,a.email,a.direccion,ffnn,id, a.telefono,a.celular,nivel,(select local from usuarios where login=a.nivel) from registro.registers a where a.activo=1 and login=$id ";
	//echo $qry;
	$res=qry($qry);
	...
	$res=mysql_query("select departamento,provincia,distrito from ubigeo where id=$r[6];");
	$ubig=mysql_fetch_row($res);
	...
	{
		$sel=""; if($temp[0]==$ubig[0]){$sel="selected";}
		$dep .= "<option value='$temp[0]' $sel>$temp[0]</option>";
	}
	...
	for($i=2000;$i>1950;$i--)
	{
		$sel=""; if($i==$fech[0]){$sel="selected";}
		$ani .= "<option $sel value='".$i."'>".$i."</option>";
	}
	...
	for($i=1;$i<=12;$i++)
	{
		$sel=""; if($i==$fech[1]){$sel="selected";}
		$sme .= "<option $sel value='".$i."'>".$tabla_mes[$i]."</option>";
		if($i==2)
	...
	$emp="";
	while($temp=mysql_fetch_row($res))
	{$sel=""; if($temp[0]==$r[9]){$sel="selected";} $emp .= "<option $sel value='".$temp[0]."'>".$temp[1]."($temp[0])</option>";}
	
	$res=mysql_query("select local from locales where local!='' and isnull(local)!=1  order by local;");
	$loc="";
	while($temp=mysql_fetch_row($res))
	{$sel=""; if($temp[0]==$r[10]){$sel="selected";} $loc .= "<option $sel value='".$temp[0]."'>".$temp[0]."</option>";}
...
<html>
	<head>
		...
		<script>
			...
			$.post("../res/users/regnew.php",{a:"old",dni:dni,nom:nom,ape:ape,ema:ema,dir:dir,depa:depa,prov:prov,dist:dist,anio:anio,mes:mes,dia:dia,tel:tel,cel:cel,emp:emp,loc:loc,cue:cue,vou:vou,mon:mon,aniod:aniod,mesd:mesd,diad:diad,pago:pago,tipo:"web"},function(data){
			rst=data[0];
			msj=data[1];
			if(rst=="1" || rst=="2" || rst=="5")
			{alert(msj); if(rst=="5"){location.reload();}}
			if(rst=="0")
			...
			$("#mes").change();
			$("#dia").val(<?php echo $fech[2];?>);
			
			$("#depa").change();
			$("#prov").val("<?php echo $ubig[1];?>");
			$("#prov").change();
			$("#dist").val("<?php echo $ubig[2];?>");
	...
	<body>
		<div class="container">
			<div class="col-sm-6">
				...
				<div class="form-group">
					<label>DNI*</label>
					<input id="dni" placeholder="DNI" class="form-control" value="<?php echo $r[0];?>">
				</div>
				<div class="form-group">
					<label>Nombre*</label>
					<input id="nom" placeholder="Nombre" class="form-control" value="<?php echo $r[1];?>">
				</div>
				<div class="form-group">
					<label>Apellido*</label>
					<input id="ape" placeholder="Apellido" class="form-control" value="<?php echo $r[2];?>">
				</div>
				<div class="form-group">
					<label>Email</label>
					<input id="ema" placeholder="Email" class="form-control" value="<?php echo $r[3];?>">
				</div>
				<div class="form-group">
					<label>Direccion*</label>
					<input id="dir" placeholder="Direccion" class="form-control" value="<?php echo $r[4];?>">
				</div>
				...
				<div class="form-group">
					...
					<input id="tel" placeholder="Telefono" class="form-control" value="<?php if($r[7]>0){echo $r[7];}?>">
				</div>
				<div class="form-group">
					<label>Celular</label>
					<input id="cel" placeholder="Celular" class="form-control" value="<?php if($r[8]>0){echo $r[8];}?>">
				</div>
			</div>
			...
			<div class="col-sm-6">
				...
				<div class="form-group">
					...
					<select id="emp" class="form-control"><?php echo $emp;?></select>
				</div>
				<div class="form-group">
					<label>Local</label>
				...
				<div class="form-group">
					...
					<select id="pago" class="form-control"><option disabled selected >--Seleccione--</option><option value="DEPOSITO">DEPOSITO</option><option value="EFECTIVO">EFECTIVO</option></select>
				</div>
		...
		<div class="text-center"><button id="save" class="btn btn-success">GUARDAR</button><a id="canc" class="btn btn-primary" href="list_web.php">Regresar</a></div>
	</body>
</html>