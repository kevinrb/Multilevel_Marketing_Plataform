<?php
	...
	if($Xpais!=604){
		header("Location: venta_acumulada.new.php");
	}
	...
	$res=qry("select estado,nivel,hora_in from user_nivel where login='$Xlogin'");
	$data=mysql_fetch_row($res);
	if($data[0]>0){
		if($data[1]>0)
		{
			$qry1="select sum(if(a.estado=1,1,0)),sum(if(a.estado=3,1,0)) from puntos  a, usuarios b where a.login=b.login and  a.login='$Xlogin' and a.tipo=3 and if(day(now())<day('$data[2]'),a.hora>concat('2015-',(month(now())-1),day('$data[2]')),a.hora>concat('2015-',month(now()),day('$data[2]')))";
			$res1 = qry($qry1);
			...
			$qry1="select sum(if(a.estado=1,1,0)),sum(if(a.estado=3,1,0)) from puntos  a, usuarios b where a.login=b.login and  a.login='$Xlogin' and a.tipo=3 and hora>'$data[2]'";
			$res1 = qry($qry1);
			$nped=mysql_fetch_row($res1);
			if($nped>0){
				...
				$arr["msg"]="Ingrese su Primer Autoconsumo";
				$arr["est"]=2;					
			}
			...
			$(window).load(function(){
				...
				$("#conf").click(function(){
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_acumulado',i:idbol},function(data){
							$(".desc").html(data.text);
							$("#acum").text(data.acum);
				...
				$("#apenv").click(function(){
					...
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv_acum",dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest},function(data){
						data[0]=data[0]-data[3];
						data[0]=Math.round(100*data[0])/100;
		...
		<div class="container">
			...
			<div class="row">
				<div class="panel panel-success">
					<div style="height:48px;" class="panel-heading">Lista Productos <strong>(<?php echo "Monto Acumulado Mensual: S/.".$Xacum;?>)</strong></div> 
