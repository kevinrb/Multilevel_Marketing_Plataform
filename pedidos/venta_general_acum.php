<?php
	...
	if($can!="0" && $lp!="0"){
		...
		$res=qry("select a.idprod,a.precio1,UPPER(a.nombre) nombre,1 pack,'' barras,0 stock,a.promo, '' precvar, '' fraccion,'' unmedida, '' unidades,if(a.codigo1 in ($idprosblo),1,0) from productos$Xpais a   where  isnull(a.nombre)!=1 and a.nombre!='' and a.estado=1 GROUP BY a.idprod order by a.nombre;");		
		$prod=array();
		while($temp=mysql_fetch_row($res)){ $prod[]=$temp; }
		$listp=json_encode($prod);
		}else{
		$qprod=qry("select a.idprod,a.precio1,UPPER(a.nombre) nombre,1 pack,'' barras,0 stock,a.promo, '' precvar, '' fraccion,'' unmedida, '' unidades,if(a.codigo1 in ($idprosblo),1,0) from productos$Xpais a where  isnull(a.nombre)!=1 and a.nombre!=''  GROUP BY a.idprod order by a.nombre;");	
		...
		<script>
			...
			$(window).load(function(){
							...
							$.post("../ajax/gproc.php",{a:"fuser_act",s:busq2.val(),c:$("#listcanales :selected").text()},function(data){
								...
								busq2.keydown(function(e){
									...
									if(tecla==13)
									{
										...
										$("#iddest").val('');
										var pais=$("#pais").val();
										$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_acumulado',tipo:"g",i:idbol,login:idcli,pais:pais,lp:$("#listpre").val()},function(data){
											$(".desc").html(data.text);
											a_cli=data.a_cli;
											a_dest=data.a_dest;
				...
				$("#savev").click(function(){
					...
					if(r==true){
						...
						$.post("../res/pedidos/venta_canales_auto.php",{
							...
							idbol:idbol,
							fpago:$("#fpago").val(),
							idruc:idruc ,razon:razon ,dir_ruc:dir_ruc ,ruc:ruc ,dir:dir ,ref:ref ,dep:dep,pro:pro,dis:dis,iddest:iddest,tpago:tpago,pago:pago,transp:transp,pais:pais,lp:$("#listpre").val()
							},function(data){
							btn.button('reset');
				...
				$("#apenv").click(function(){
					...
					var pais=$("#pais").val();
					$.post("../res/pedidos/venta_canales_auto.php",{a:"calcenv_acum",login:idcli,dir:dir,ref:ref,dep:dep,pro:pro,dis:dis,i:idbol,tip:"u",iddest:iddest,pais:pais,lp:$("#listpre").val()},function(data){
						$("#savev").prop("disabled",false);
						$("#totpeso").val(data[2]);
						$("#totenv").val(data[1]);
						$("#totmon").val(data[0]);
	...
	<body>
		<div class="container">
			...
			<div class="row">
				...
				<input id="pais" type='hidden' value='<?php echo $Xpais;?>' />
				<input id="listpre" type='hidden' value='1' />
									...
									<div id="divmov" class="col-sm-12">
										...
										<div class="input-group">
											<span class="input-group-addon">
												<input type="checkbox"  id="transp" checked value="1" />
											</span>
