<?php
	...
	if($Xpais!=604){
		header("Location: venta_auto.new.php");
	}
	...
	$res=qry("select estado,nivel,hora_in,(concat(date(hora_in),' 23:59:59') + INTERVAL (TIMESTAMPDIFF(MONTH,concat(date(hora_in),' 23:59:59'),now())) month),deuda from user_nivel where login='$Xlogin'");
			...
			$(window).load(function(){
				...
				$("#conf").click(function(){
					if($(".list").length>0){
						...
						////calculo de niveles//
						$.post("../res/pedidos/venta_canales_auto.php",{a:'descuento_unico',i:idbol},function(data){
							$(".desc").html(data.text);
